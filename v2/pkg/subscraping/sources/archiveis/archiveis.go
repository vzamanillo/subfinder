...
func (a *agent) enumerate(ctx context.Context, baseURL string) {
	...
	if err != nil {
		a.Results <- subscraping.Result{Source: a.Name, Type: subscraping.Error, Error: err}
	...
	for _, subdomain := range a.Session.Extractor.FindAllString(src, -1) {
		a.Results <- subscraping.Result{Source: a.Name, Type: subscraping.Subdomain, Value: subdomain}
	}
